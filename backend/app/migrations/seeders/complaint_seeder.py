import random
from app.models import User, Role, Complaint
...
class ComplaintSeeder(BaseSeeder):
    OBJECT_NUMBER = 50
    REQUIRED_SEEDERS = [UserSeeder, HotelSeeder, BookingSeeder]
    ...
    def run(self, stdout, _):
        ...
        for i in range(self.OBJECT_NUMBER):
            ...
            while user.bookings.count() == 0:
                user = random.choice(users)
            hotel = random.choice([booking.room.hotel for booking in user.bookings.all()])
